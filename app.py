...
st.header (" Car Prediction ML Model (by Asad Rahmani)")
...
name = st.selectbox('Select Brand of your Car ' , car_data['name'].unique())
...
mileage = st.number_input("Car Milleage ")
owner = st.selectbox("Enter Owner Type ",car_data['owner'].unique())
engine = st.slider (' Engine Capacity (CC)  ',700,5000)
max_power  = st.slider ('Max Power  ', 0,200)
seats = st.slider ('No of Seat  ', 2,12)
...
if st.button(" Predict"):
    ...
    st.markdown('Car Price will be ' + str(car_price[0]))
